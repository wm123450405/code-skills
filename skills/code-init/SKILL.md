...
- 建立版本工作空间(基线版本)
- 把**现有代码当成"已交付的需求"**登记到 `require/EXISTING-NNN/`,使后续所有改动都能走 `code-require → code-design → code-plan → code-it → code-unit → code-review` 的标准化流程
...
- **若 false(`rules/` 不存在或为空)**,汇报:
  > ⚠️ `./assistants/rules/` 目录为空,后续 `code-design` / `code-plan` / `code-it` / `code-unit` / `code-review` 都会从这里读规范作为约束,缺少规范会让这些技能退化为"无约束模式"。
  >
...
| `code-design` / `code-plan` / `code-it` / `code-unit` / `code-review` | 后续都基于 `<初始版本号>/` 下的产物工作,无改动 |
