...
- **影响**:本设计让 `code-unit` 守卫在"项目不可测"时自动跳过,**正好匹配**本仓库现状
- **无需纠正**:本设计**不**需要纠正任何现状偏离(本仓库"不可测"是设计预期,V0.0.1 21/21 任务已"测试状态=不适用")
...
## 4. 用户授权的偏离
- **偏离 0 条**
- 本概要设计**100% 沿用**需求 FR / NFR,**0** 偏离

## 5. 规范变更响应(增量更新时填写)
- **不适用**(本设计为首次概要设计,无规范变更)
...
| 修改后 frontmatter **不**变 | ✅ 本设计**不**改 frontmatter |
...
| --- | --- |
| SKILL.md 在技能根目录 | ✅(既有) |
| 资源在 `templates/` / `checklists/` / `guidelines/` 子目录 | ✅(既有 3 份 templates/,本设计**不**新增) |
| 修改后**不**引入新子目录 | ✅ 本设计**不**新增子目录 |
...
| 检查项 | 结果 |
| --- | --- |
| 本需求是否改中英 README | ❌ 无(本需求**不**改 README) |
| 结论 | ✅ **不触达** |
...
| 检查项 | 结果 |
| --- | --- |
| 本需求是否产出新编码 | ❌ 无(本需求**不**产出新需求 / 任务 / 缺陷编码) |
| 结论 | ✅ **不触达** |
...
| 检查项 | 结果 |
| --- | --- |
| 本需求是否追溯重命名 | ❌ 无(本需求**不**涉及 EXISTING-NNN) |
| 结论 | ✅ **不触达** |
...
| `code-version` | ❌ | 独立 |
| `code-rule` | ❌ | 独立 |
| `code-require` | ❌ | 独立(本需求**不**触发其变更) |
| `code-design` | ❌ | 独立(本需求**不**触发其变更) |
| `code-plan` | ❌ | 独立(本需求**不**触发其变更) |
| `code-it` | ❌ | 独立(本需求**不**触发其变更) |
| `code-fix` | ❌ | 独立 |
| `code-review` | ❌ | 独立 |
| `code-dashboard` | ❌ | 沿用既有"不适用"枚举,**0** 改动 |
| `code-publish` | ❌ | 沿用既有"测试∈{已运行-通过, 不适用}",**0** 改动 |
| `code-auto` | ❌ | 协同关系,本需求**不**改其 SKILL.md |
| `code-merge` | ❌ | 独立 |
...
- **100% 字节级保留**既有 `code-unit/SKILL.md` frontmatter + 既有 17 章节中除"步骤 0 之前"插入"步骤 0a"外的所有内容
