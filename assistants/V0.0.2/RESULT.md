...
| M1-REQ-00010-2:本需求可发布 | M1-REQ-00010-1 + T-002(REQ-00010) | **2 任务开发状态=已完成 且 测试状态=不适用**,13/13 INV 100% 自检通过 + 看板 5 处一致(任务清单 2 行 + 详细设计汇总 + 里程碑 2 个 + 文档头 + 变更记录)+ 0 触发 `dashboard-conventions §规则 1` 3 处同步 + 0 修改其他 11 个 `code-*` SKILL.md + 0 修改 `marketplace.json` / `plugin.json` + 0 新增三方依赖 + `code-it` 步骤 0a 守卫可触发(未来调 `code-auto` 跑一个含前置未完成任务的场景验证) | 待开始 | 2026-06-06 | — |
...
| `TASK-REQ-00010-00002` | REQ-00010 | 文档 | 详细设计 | [文档] 13 项不变量自检(INV-1~13) + 偏差日志 + 看板同步 + 收尾 | 待开始 | 不适用 | `assistants/V0.0.2/RESULT.md` + `code/TASK-REQ-00010-00002/{RESULT,work-log,deviations}.md` | — | — | T-001 |
