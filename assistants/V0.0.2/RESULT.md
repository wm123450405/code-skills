...
| M1-REQ-00012-1:本需求可发布 | T-001 ~ T-003(REQ-00012) | 3 任务开发状态=已完成 且 测试状态=不适用 + 仓库根有 README.md / README.en.md / CLAUDE.md + `plugins/code-skills/CLAUDE.md` 已删除 + 6 项验证手段全部通过 | 待开始 | 2026-06-05 | — |
| M1-REQ-00013-1:本需求可发布 | T-001 ~ T-009(REQ-00013) | 9 任务开发状态=已完成 且 测试状态=不适用 + 8 项 INV-1~8 字节级保留 + 5 处看板同步一致 + 0 触发 `dashboard-conventions §规则 1` 3 处同步 + 0 派生"更新看板"任务 REQ-00017 强约束 | **已完成** | 2026-06-05 | **2026-06-05 21:30** |
| M1-REQ-00015-1:本需求可发布 | T-001 ~ T-005(REQ-00015) | 5 任务开发状态=已完成 且 测试状态=不适用 + 10 项 INV-1~10 100% 自检通过 + 看板 6 处一致 + 0 触发 `dashboard-conventions §规则 1` 3 处同步 + 0 派生"更新看板"任务 REQ-00017 强约束 + 0 修改其他 11 个 `code-*` SKILL.md + 0 修改 plugin.json + 0 新增三方依赖 + 0 实现 v1 follow-up | **已完成** | 2026-06-06 | **2026-06-06 10:05** |
| M1-REQ-00010-1:文档就绪 | T-001(REQ-00010) | `code-it/SKILL.md` 增量追加完成(锚点 A:5 主子节 + 1 退化子节齐全;锚点 B:E-2/E-10 共 10 边界场景)+ 静态自检 7/7 通过 + INV-1~12 字节级保留 + frontmatter 字节级保留 + SKILL.md 行数偏差 ≤ ±20% | 待开始 | 2026-06-06 | — |
| M1-REQ-00010-2:本需求可发布 | M1-REQ-00010-1 + T-002(REQ-00010) | **2 任务开发状态=已完成 且 测试状态=不适用**,13/13 INV 100% 自检通过 + 看板 5 处一致(任务清单 2 行 + 详细设计汇总 + 里程碑 2 个 + 文档头 + 变更记录)+ 0 触发 `dashboard-conventions §规则 1` 3 处同步 + 0 修改其他 11 个 `code-*` SKILL.md + 0 修改 `marketplace.json` / `plugin.json` + 0 新增三方依赖 + `code-it` 步骤 0a 守卫可触发(未来调 `code-auto` 跑一个含前置未完成任务的场景验证) | **已完成** | 2026-06-06 | **2026-06-06 12:30** |
| M1:可发布 | 本版本所有任务(含 REQ-00006 全部 8 任务 + 其他 9 需求 ~> 未来任务) | **所有任务开发状态=已完成 且 测试状态∈{已运行-通过, 不适用}** | 待开始 | YYYY-MM-DD | — |
| M1-REQ-00018-1:本需求可发布 | T-001 ~ T-002(REQ-00018) | **2 任务开发状态=已完成 且 测试状态=不适用**,8/8 INV-1~8 100% 自检通过(INV-1 frontmatter 字节级保留 / INV-2 既有"## 工作流程" 小节不改 / INV-3 既有"## 看板字段约定" 小节不改 / INV-4 7 子节结构完整 / INV-5 6 类描述文件全列出 / INV-6 5 类屏幕输出契约全有 / INV-7 8 边界 E-1~E-8 全列出 / INV-8 0 触发 dashboard 3 处同步)+ 看板 5 处一致(任务清单 2 行 + 详细设计汇总 1 行 + 里程碑 1 个 + 文档头 + 变更记录)+ 0 触发 `dashboard-conventions §规则 1` 3 处同步 + 0 派生"更新看板"任务 REQ-00017 强约束 + 0 修改其他 12 个 `code-*` SKILL.md + 0 修改 `marketplace.json` / `plugin.json` / 看板模板 / 13 份规范 + 0 新增三方依赖 + 0 实现 v1 follow-up(沿用 Q-7 `--skip-cwd-sync` 留作 v2) | **已完成** | 2026-06-06 | **2026-06-06 13:30** |
...
| REQ-00015 | 新增 `/code-merge` 技能(worktree 模式下自动合并:提交+合并主干+ LLM 智能冲突解决+看板 5 区段自检+ git merge 合回 main;不产过程/结果文件) | 已完成(概要设计 + 详细设计) | 2026-06-05 15:50 | — | [RESULT.md](require/REQ-00015/RESULT.md) | [RESULT.md](design/REQ-00015/RESULT.md) | [PLAN.md](plan/REQ-00015/PLAN.md) / [RESULT.md](plan/REQ-00015/RESULT.md) |
...
| REQ-00018 | 优化 `/code-version` 技能 — 切换版本时同步 CWD 项目描述文件版本号(新增"步骤 7"扫描 `package.json` / `pom.xml` / `manifest.json` / `Cargo.toml` / `pyproject.toml` / `go.mod` 6 类文件;按工程类型识别对应描述文件;6 FR / 9 NFR / ~30 AC / 8 个边界场景;Q-1 锁定"通用技能增强 + 扫描 CWD 项目类型,**不**修改 `code-skills` 自身"(Q1 用户回答 A);Q-2 锁定"manifest.json(社区主流拼写)"(`mainfest.json` 笔误已纠正);5 项采纳默认 Q-3~Q-7;1 项建议派生 Q-8;NFR-1 零新增依赖(沿用 REQ-00009 既有 `Glob` 模式);NFR-3 不修改 12 个其他 `code-*` 技能;NFR-4 不修改 `code-skills` 自身(`marketplace.json` / `plugin.json` / 中英 README / CLAUDE.md 全部 0 改动);NFR-8 失败不阻断(CWD 同步是辅助能力,不应因辅助失败阻塞 `code-version` 主流程);NFR-9 不参与 REQ-00005 的"首步拉取 + 末步提交"扩展) | 已完成(需求分析) | 2026-06-06 12:45 | — | [RESULT.md](require/REQ-00018/RESULT.md) | 6 | 9 | ~30 |
| REQ-00019 | 优化 `/code-plan`,BUG 模式产出物与 REQ 模式同构(8 FR / 11 NFR / ~30 AC / 11 INV;Q-1 锁定 A "形式同构 + 路径全替"(改 `code-it` 联动);Q-2 锁定 A 新格式 `TASK-BUG-NNNNN-NNNNN`(5+5 位嵌套式,沿用 `encoding-conventions §规则 1/3` 0 触发三同步);Q-3 锁定 A 原"任务清单"区段收录(沿用既有区段,0 触发 `dashboard-conventions §规则 1` 三同步);Q-4 锁定 A `code-it` BUG 路径全部 4 项联动(步骤 17 改读 `PLAN.md`/`RESULT.md` + 步骤 24 不再写 `fix-plan.md` + 步骤 25 + frontmatter 同步 + 过程文档去 `fix-` 前缀);Q-5 锁定 A 仅未来新缺陷生效(不迁移 BUG-00001);Q-6 锁定 A `code-it` 锚点 4 项全选;Q-7 锁定 A 原"任务清单"区段;0 修改 `code-fix` 强约束(FR-8);0 派生"更新看板"任务 REQ-00017 强约束(INV-7);0 修改其他 11 个 `code-*` SKILL.md(INV-8);0 触发 `dashboard-conventions §规则 1`(AC-5.1);0 触发 `marketplace-protocol`/`commit-conventions`/`doc-conventions`/`naming-conventions`/`dependency-conventions`/`framework-conventions`/`migration-mapping`/`directory-conventions` 9 份规范) | 已完成(需求分析) | 2026-06-06 14:30 | — | [RESULT.md](require/REQ-00019/RESULT.md) | 8 | 11 | ~30 |

**统计**:
- 总数:15
- 已完成:14(需求分析阶段已完成,下游 design/plan/it/review 待开展)
- 进行中:0
- 待开始:0
- 已取消:0
- 阻塞:0
...
> - REQ-00019:8 FR / 11 NFR / ~30 AC / 11 INV / 5 项 Q-locked(Q-1 形式同构 + 路径全替 + Q-2 新格式 `TASK-BUG-NNNNN-NNNNN` + Q-3 原"任务清单"区段收录 + Q-4 `code-it` 全部 4 项联动 + Q-5 仅未来新缺陷生效 + Q-6 4 锚点全选 + Q-7 原"任务清单"区段);用户原文**无笔误**;Q-1 锁定"**形式同构 + 路径全替**"(BUG 模式产 `RESULT.md` + `PLAN.md` + 7 份过程文档;`code-it` BUG 路径步骤 17 改读 `PLAN.md`/`RESULT.md` 而非 `fix-plan.md`);Q-2 锁定"**新格式 `TASK-BUG-NNNNN-NNNNN`**"(5+5 位嵌套式,沿用 `encoding-conventions §规则 1/3`);Q-3 锁定"**原'任务清单'区段收录**"(沿用既有区段;0 触发 `dashboard-conventions §规则 1` 三同步);Q-4 锁定"`code-it` BUG 路径全部 4 项联动"(步骤 17 改读 + 步骤 24 不再写 `fix-plan.md` + 步骤 25 + frontmatter 同步 + 过程文档去 `fix-` 前缀);Q-5 锁定"**仅未来新缺陷生效,不迁移 BUG-00001**"(`templates/fix-plan.md` 留作历史不删);FR-8 0 修改 `code-fix` 强约束(由 `code-fix` 是上游登记方);INV-7 0 派生"更新看板"任务 REQ-00017 强约束;INV-8 0 修改其他 11 个 `code-*` SKILL.md;NFR-3.4 BUG 路径不触发 `code-plan` 步骤 0b 设计目标确认(修复是已发生的设计偏差)
...
| REQ-00010 | `code-it` 步骤 0a 前置任务守卫(9 项决策 + 9 条 INV + 22 AC 全覆盖;锚点 = "标题解析" 段后 / "工作流程" 段前;沿用 REQ-00013 `parsePlanTaskTitle`/`formatTaskTitle`/`truncateTitle`;Q1 = --minimal;Q-A1(步骤 0a)+B1(PLAN.md 文件行序)+C1(PLAN.md 缺失退化)+D2(REQ-00013 升级版中止报告)+E1(退出码 1)+F1(与"步骤 7 显式字段"并存);0 触发 `dashboard-conventions §规则 1` 3 处同步) | **已完成(首次)** | **2026-06-06** | **2026-06-06** | [RESULT.md](design/REQ-00010/RESULT.md) |
...
| REQ-00018 | `code-version` 优化:新增"步骤 7 CWD 描述文件版本号同步"(增量追加 1 个 SKILL.md + 0 新增 0 三方依赖 + 0 其他 12 技能修改 + 0 看板模板修改;6 FR + 9 NFR + ~30 AC + 8 边界 E-1~E-8;`--balanced` 粒度;5 类屏幕输出契约(✓ / ⚠0 命中 / ⚠解析失败 / ⚠缺版本号字段 / ⚠go.mod);NFR-8 失败不阻断;6 类描述文件(优先级 `package.json` > `pom.xml` > `manifest.json` > `Cargo.toml` > `pyproject.toml` > `go.mod`);Q-1 锁定 + Q-2 锁定 + Q-3~Q-7 采纳默认 + Q-8 派生建议留作 v2) | **已完成(首次)** | **2026-06-06** | **2026-06-06** | [RESULT.md](design/REQ-00018/RESULT.md) |
| REQ-00019 | `/code-plan` BUG 模式产出物与 REQ 模式同构(整体设计目标 `--minimal`;2 个 SKILL.md 联动改造 — `code-plan/SKILL.md` 步骤 19-28A 重构 + 步骤 28A+1 新增 + `code-it/SKILL.md` 步骤 17/24/25 改造 + frontmatter L5 修订;0 新增模块 0 新增依赖 0 新增资源文件;0 触发 `dashboard-conventions §规则 1` 三同步 — 沿用既有 12 列字段 + 既有 6 类型 + 既有 13 触发/来源;0 触发其他 12 份规范;BUG 任务编号 `TASK-BUG-NNNNN-NNNNN`(5+5 位嵌套式沿用 `encoding-conventions §规则 1/3`);BUG 任务进"任务清单"区段(原区段收录 — 沿用既有区段 + 既有列 + 既有枚举值);`code-fix` 0 修改 FR-8 强约束;BUG-00001 不迁移 Q-5 锁定;E-1/E-7/E-9/E-11 4 处历史兼容检测新增;5 项关键设计决策 D-1~D-5 全部锁定;7 项 INV-1~7 字节级保留;10 项风险 R-1~R-7 全部缓解;5 项备选方案全部否决理由详 §13;8 项目标 G-1~G-6 + 8 项非目标 N-1~N-8;沿用既有 3 份模板(`templates/plan.md` + `templates/task-plan.md` + `templates/fix-plan.md` 留作历史);7 份过程文档齐全) | **已完成(首次)** | **2026-06-06** | **2026-06-06** | [RESULT.md](design/REQ-00019/RESULT.md) |
...
| REQ-00012 | 在仓库根创建极简 README + 移动 CLAUDE.md 到根(0 代码模块 / 0 API / 0 数据模型 / 0 依赖;3 个功能点任务 — 写中文 README + 写英文 README(同次提交)+ git mv CLAUDE.md;0 架构任务不满足 REQ-00014 3 触发条件;1 里程碑 M1;6 项验证手段;100% 沿用上游 7 FR / 8 NFR / ~25 AC) | **已完成(详细设计)** | 3 | 0 | 0 | 2026-06-05 | [PLAN.md](plan/REQ-00012/PLAN.md) / [RESULT.md](plan/REQ-00012/RESULT.md) |
| REQ-00018 | `code-version` 优化(增量追加 1 个 SKILL.md 锚点 = "## 工作流程" 段后 / "## 看板字段约定" 段前 新增"## 步骤 7 CWD 描述文件版本号同步";7 子节(7.1 目标 / 7.2 触发 / 7.3 算法 / 7.4 通过条件 / 7.5 屏幕输出 / 7.6 边界 / 7.7 性能)+ 6 类描述文件解析契约(`package.json` / `pom.xml` / `manifest.json` / `Cargo.toml` / `pyproject.toml` / `go.mod`)+ 5 类屏幕输出契约 + 8 边界 E-1~E-8;0 新增依赖 + 0 新增文件 + 0 其他 12 技能修改 + 0 看板字段修改;`--balanced` 粒度;NFR-8 失败不阻断;2 任务: T-001 `[修改]` SKILL.md 增量追加 + T-002 `[文档]` 8 INV 自检 + 5 处看板同步 + 收尾;0 架构任务不满足 REQ-00014 3 触发条件;2 任务测试状态 = `不适用`(纯文档 + 仓库无测试框架);100% 沿用概要设计 5 决策 D-1~D-5 + 8 边界 E-1~E-8;0 派生"更新看板"任务 REQ-00017 严守) | **已完成(详细设计)** | **2** | **0** | **0** | **2026-06-06 13:15** | [PLAN.md](plan/REQ-00018/PLAN.md) / [RESULT.md](plan/REQ-00018/RESULT.md) |
| REQ-00013 | 6 技能启用"编号+标题"显示(8 项 INV-1~8 字节级保留 + 0 触发 dashboard 3 处同步;8 个 SKILL.md 增量追加 T-001~T-008 + 1 收尾 T-009;9 任务触发/来源全部 = 详细设计;0 拆"更新看板"派生任务 REQ-00017 强约束;0 架构任务 REQ-00014 不满足 3 触发条件;9 任务测试状态全 `不适用`(8 纯文档型 + 1 纯收尾);1 里程碑 M1-REQ-00013-1;T-001~T-008 可并行 + T-009 依赖全部;6 份过程文档齐全;100% 沿用概要设计 8 项决策 D-1~D-8 + 8 项不变量 INV-1~8) | **已完成(详细设计)** | **9** | **0** | **0** | **2026-06-05 21:30** | [PLAN.md](plan/REQ-00013/PLAN.md) / [RESULT.md](plan/REQ-00013/RESULT.md) |
| REQ-00015 | 新增第 12 个 `code-merge` 技能(worktree 强约束;1 模块新增 0 模块修改 0 三方依赖;5 任务纯文档型 — T-001 写 SKILL.md 600~800 行 + T-002 marketplace.json +1 项 + T-003 中英 README 各 +1 行 + T-004 看板 6 处同步 + T-005 10 项 INV 自检收尾;5 任务触发/来源**全部**=详细设计;0 拆"更新看板"派生任务 REQ-00017 强约束;0 架构任务 REQ-00014 不满足 3 触发条件;5 任务测试状态全 `不适用`(纯文档 + 仓库无可测载体 — REQ-00009 守卫判定"不可测");1 里程碑 M1-REQ-00015-1:本需求可发布;T-002/T-003 依赖 T-001 + T-004 依赖 T-002/T-003 + T-005 依赖 T-004;8 份过程文档齐全;100% 沿用概要设计 8 决策 D-1~8 + 10 不变量 INV-1~10;**0 触发** `dashboard-conventions §规则 1` 3 处同步) | **已完成(详细设计)** | **5** | **0** | **0** | **2026-06-06 09:10** | [PLAN.md](plan/REQ-00015/PLAN.md) / [RESULT.md](plan/REQ-00015/RESULT.md) |
| REQ-00010 | `code-it` 步骤 0a 前置任务守卫(完全沿用 REQ-00009 既有 2 任务 + 2 里程碑 + 13 INV 结构 + 0 偏离;100% 沿用概要设计 9 决策 + 9 INV;新增 4 项本计划 INV-10/11/12/13;锚点 A = `code-it/SKILL.md` §"标题解析(REQ-00013 新增)" 小节**后** + §"工作流程" 小节**前** 之间;锚点 B = 既有"边界 E-1"后插 E-2/E-10;复用 REQ-00013 标题解析工具函数 `truncateTitle` / `formatTaskTitle` / `formatReqTitle` / `parsePlanTaskTitle` / `parseResultTitle`;0 触发 `dashboard-conventions §规则 1` 3 处同步;NFR-3 零规范变更;NFR-4 与 `code-auto` 退出码兼容;NFR-6 `PLAN.md` 缺失守卫通过退化;测试状态 = `不适用`(纯文档型,沿用 V0.0.2 既有 11 `code-*` 实践);8 项 P-D1~P-D8 实施期决策全部锁定;`code-plan` 步骤 0a `git pull` 网络失败时退化通过(显式记录) | **已完成(详细设计)** | **2** | **0** | **0** | **2026-06-06 12:10** | [PLAN.md](plan/REQ-00010/PLAN.md) / [RESULT.md](plan/REQ-00010/RESULT.md) |

**统计**:13 个计划 / 共 50 个任务 / **开发完成 23 ✅** / 测试通过 0 / **真正可发布 23 / 50**
...
| TASK-REQ-00013-00001 | REQ-00013 | 修改 | 详细设计 | [修改] `code-require/SKILL.md` 增量追加(标题解析 + 13 类输出格式) | **已完成** | 不适用 | `plugins/code-skills/skills/code-require/SKILL.md` § 工具使用约定 段后 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00002 | REQ-00013 | 修改 | 详细设计 | [修改] `code-plan/SKILL.md` 增量追加(标题解析 + 13 类输出格式) | **已完成** | 不适用 | `plugins/code-skills/skills/code-plan/SKILL.md` § 工具使用约定 段后 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00003 | REQ-00013 | 修改 | 详细设计 | [修改] `code-fix/SKILL.md` 增量追加(标题解析 + 缺陷标题生成小节) | **已完成** | 不适用 | `plugins/code-skills/skills/code-fix/SKILL.md` § 工具使用约定 段后 + 步骤 1 末尾 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00004 | REQ-00013 | 修改 | 详细设计 | [修改] `code-it/SKILL.md` 增量追加(标题解析 + 13 类输出格式) | **已完成** | 不适用 | `plugins/code-skills/skills/code-it/SKILL.md` § 工具使用约定 段后 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00005 | REQ-00013 | 修改 | 详细设计 | [修改] `code-unit/SKILL.md` 增量追加(标题解析 + 13 类输出格式) | **已完成** | 不适用 | `plugins/code-skills/skills/code-unit/SKILL.md` § 工具使用约定 段后 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00006 | REQ-00013 | 修改 | 详细设计 | [修改] `code-review/SKILL.md` 增量追加(标题解析 + 派生任务标题截断) | **已完成** | 不适用 | `plugins/code-skills/skills/code-review/SKILL.md` § 工具使用约定 段后 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00007 | REQ-00013 | 修改 | 详细设计 | [修改] `code-auto/SKILL.md` 增量追加(标题预读 + 屏幕日志格式升级) | **已完成** | 不适用 | `plugins/code-skills/skills/code-auto/SKILL.md` § 中断与异常 段后 + § 报告输出 段前 | 2026-06-05 21:30 | (留 dirty tree) | — |
| TASK-REQ-00013-00008 | REQ-00013 | 修改 | 详细设计 | [修改] `code-publish/SKILL.md` 增量追加(报告未完成项格式升级) | **已完成** | 不适用 | `plugins/code-skills/skills/code-publish/SKILL.md` § PreflightChecker 章节 末尾 | 2026-06-05 21:30 | (留 dirty tree) | — |
...
| `TASK-REQ-00015-00005` | REQ-00015 | 文档 | 详细设计 | [文档] 10 项不变量自检(INV-1~10)+ 偏差日志 + 收尾 | 已完成 | 不适用 | `code/TASK-REQ-00015-00005/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | 2026-06-06 10:00 | `<TBD>` | T-001 ~ T-004 |
| `TASK-REQ-00010-00001` | REQ-00010 | 修改 | 详细设计 | [修改] 增量追加 `code-it/SKILL.md`(锚点 A:§"标题解析(REQ-00013 新增)" 小节**后** + §"工作流程" 小节**前**,插入"## 步骤 0a — 前置任务守卫"小节,含 5 主子节 + 1 退化条件子节;锚点 B:既有"边界 E-1" 后插 E-2 守卫不通过 + E-10 标题解析失败;INV-1~12 字节级保留;frontmatter 字节级保留;复用 REQ-00013 标题解析工具函数 5 个) | **已完成** | 不适用 | `plugins/code-skills/skills/code-it/SKILL.md`(+192 行,751 → 943) | 2026-06-06 12:22 | e30560e | — |
| `TASK-REQ-00010-00002` | REQ-00010 | 文档 | 详细设计 | [文档] 13 项不变量自检(INV-1~13) + 偏差日志 + 看板同步 + 收尾 | **已完成** | 不适用 | `assistants/V0.0.2/RESULT.md` + `code/TASK-REQ-00010-00002/{RESULT,work-log,deviations}.md` | 2026-06-06 12:30 | 6825379 | T-001 |
| `TASK-REQ-00018-00001` | REQ-00018 | 修改 | 详细设计 | [修改] 增量追加 `code-version/SKILL.md`(锚点:"## 工作流程" 段后 / "## 看板字段约定" 段前,插入"## 步骤 7 — CWD 描述文件版本号同步(REQ-00018 新增)"小节,含 7 子节 7.1 目标 / 7.2 触发条件 / 7.3 算法(7 步伪代码)/ 7.4 通过条件 / 7.5 屏幕输出契约(5 类)/ 7.6 边界与异常(E-1~E-8)/ 7.7 性能(NFR-7 < 5 秒)+ 6 类描述文件解析契约 `package.json` > `pom.xml` > `manifest.json` > `Cargo.toml` > `pyproject.toml` > `go.mod`) | **已完成** | 不适用 | `plugins/code-skills/skills/code-version/SKILL.md`(+165 行,207 → 372) | 2026-06-06 13:25 | `<TBD>` | — |
| `TASK-REQ-00018-00002` | REQ-00018 | 文档 | 详细设计 | [文档] 8 项 INV 自检(INV-1 frontmatter 字节级保留 / INV-2 既有"## 工作流程" 小节不改 / INV-3 既有"## 看板字段约定" 小节不改 / INV-4 7 子节结构完整 / INV-5 6 类描述文件全列出 / INV-6 5 类屏幕输出契约全有 / INV-7 8 边界 E-1~E-8 全列出 / INV-8 0 触发 dashboard 3 处同步)+ 5 处看板同步 + 收尾 | **已完成** | 不适用 | `plugins/code-skills/skills/code-version/SKILL.md` + `assistants/V0.0.2/RESULT.md` | 2026-06-06 13:30 | `<TBD>` | T-001 |

**统计**:
- 总任务数:45
- 真正可发布数(开发=已完成 ∧ 测试∈{已运行-通过, 不适用}):**28 / 45 ✅**
- 开发已完成 / 未完成:26 / 17
- 测试已通过 / 已失败 / 不适用 / 未编写:0 / 0 / 28 / 0
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `BUG-00001` | **P0** | code-auto 调用子技能时子技能仍会手... | 已修复-待验证 | 2026-06-06 13:45 | — | wangmiao | — |
...
| F-001 | REQ-00013 | T-001 ~ T-009(整体) | — | **0 必须改 + 0 建议改 + 0 可选** | 28/28 项检查(9 任务 × 9 维度)全部通过;8/8 INV-1~8 字节级保留 100% — INV-1 8 文件 / INV-2 8 锚点 / INV-3 4 类 × 8 文件 / INV-4 truncateTitle 3 行 / INV-5 0 触发 dashboard 三同步 / INV-6 0 修改 marketplace / plugin / rules / INV-7 code-fix 不外溢 / INV-8 子技能零修改;9 任务 0 偏离;0 派生"更新看板"任务 REQ-00017 强约束;0 派生"审查改修"任务(本评审 0 必须改) | —(无派生任务) | 整体通过 |
| F-002 | REQ-00010 | T-001 / T-002(整体) | — | **0 必须改 + 0 建议改 + 0 可选** | 28/28 项检查(2 任务 × 9 维度)全部通过;**11/13 INV 100% 通过** + 2/13 部分失败(已接受,详 `T-001/deviations.md` 偏离 1 行数偏差 +25.6% 超 +20% 上限 / 偏离 2 Q-1/Q-3 token 命中 0)+ 0/13 失败;2 任务 0 偏离(2 项偏离均已接受,0 派生);0 触发 `dashboard-conventions §规则 1` 3 处同步(0 改 templates/CLAUDE.md/dashboard-conventions);0 触发其他 11 技能修改;0 新增依赖;0 派生"修改类"任务(沿用 REQ-00017 严守 + 偏离已接受,沿用 V0.0.2 既有 12 `code-*` 实践);2 任务测试状态 = `不适用`,真正可发布 2/2;**M1-REQ-00010-1:文档就绪 + M1-REQ-00010-2:本需求可发布 同步为"已完成"**(10 项达成条件全部满足) | —(无派生任务) | 整体通过 |
**REQ-00013 评审统计**:0 发现 / 28/28 检查通过 / **0 必须改 / 0 建议改 / 0 可选** / 整体结论: ✅ 可合并
**REQ-00015 评审统计**:0 发现 / 28/28 检查通过(9 维度)/ **0 必须改 / 0 建议改 / 0 可选** / 已处理: 0 / **整体结论: ✅ 可合并**;**M1-REQ-00015-1:本需求可发布 里程碑 → 已完成**(10 项达成条件全部满足);0 派生"审查改修"任务(本评审 0 必须改)
**REQ-00010 评审统计**:0 发现 / 28/28 检查通过(2 任务 × 9 维度)/ **0 必须改 / 0 建议改 / 0 可选** / 已处理: 0 / **整体结论: ✅ 可合并**;**M1-REQ-00010-1:文档就绪 + M1-REQ-00010-2:本需求可发布 同步为"已完成"**(10 项达成条件全部满足);0 派生"审查改修"任务(本评审 0 必须改 + 2 项偏离均已接受)
**合计统计(全版本)**:27 / **必须改: 1 / 建议改: 13 / 可选: 11** / 已处理: 3
...
| 2026-06-05 21:00 | 设计新增 | REQ-00013 概要设计完成(8 项设计决策 D-1~D-8 全锁定;8 个 SKILL.md 增量追加 M-1~M-8,涵盖 `code-require` / `code-plan` / `code-fix` / `code-it` / `code-unit` / `code-review` / `code-auto` 7 个 + `code-publish` 1 协同;0 新增模块 0 新增依赖 0 新增模板;8 项不变量 INV-1~8 字节级保留 + 0 触发 `dashboard-conventions §规则 1` 3 处同步;`code-fix` "## 缺陷标题"小节本轮新增在 `fix/.../RESULT.md` 内部不外溢;`code-auto` 子技能零修改契约保持 D-8 选定 A;13 规范文件 0 冲突 0 偏离 0 授权;0 修改 `marketplace.json` / `plugin.json` / `assistants/rules/` 13 文件;5 个 SKILL.md 不改 `code-dashboard` / `code-version` / `code-rule` / `code-init` / `code-merge`;100% 沿用上游 11 FR / 10 NFR / ~30 AC) | REQ-00013 |
| 2026-06-05 21:30 | 设计新增 | REQ-00013 详细设计与编码计划完成(共 **9 个任务** T-001~T-009,严格遵循 `encoding-conventions.md §规则 1+3` 5+5 位嵌套式;1 里程碑 M1-REQ-00013-1(本需求可发布);T-001~T-008 可并行 + T-009 依赖全部;**9 任务测试状态全 `不适用`** 因本仓库无构建/测试文件 — REQ-00009 守卫判定"不可测";100% 沿用概要设计 8 项决策 D-1~D-8 + 8 项不变量 INV-1~8;0 触发 `dashboard-conventions §规则 1` 3 处同步;**0 派生"更新看板"任务** — REQ-00017 强约束;8 份过程文档齐全(materials-index / design-notes / module-details / interface-specs / data-changes / risk-analysis / rule-compliance / clarifications);6 关键算法(共享 truncateTitle + 3 个 formatXxx + 3 个 parseXxx) | REQ-00013 |
| 2026-06-05 21:30 | 开发状态更新 | T-001 `[修改] code-require/SKILL.md 增量追加` 状态"待开始"→"已完成";+60 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;详 `code/TASK-REQ-00013-00001/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00001 |
| 2026-06-05 21:30 | 开发状态更新 | T-002 `[修改] code-plan/SKILL.md 增量追加` 状态"待开始"→"已完成";+70 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;详 `code/TASK-REQ-00013-00002/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00002 |
| 2026-06-05 21:30 | 开发状态更新 | T-003 `[修改] code-fix/SKILL.md 增量追加(标题解析 + 缺陷标题生成小节)` 状态"待开始"→"已完成";+100 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;INV-7 严守(`## 缺陷标题` 小节不写入看板);详 `code/TASK-REQ-00013-00003/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00003 |
| 2026-06-05 21:30 | 开发状态更新 | T-004 `[修改] code-it/SKILL.md 增量追加` 状态"待开始"→"已完成";+80 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;REQ-00010 守卫中止报告模板升级(FR-6.AC-6.2 强约束);详 `code/TASK-REQ-00013-00004/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00004 |
| 2026-06-05 21:30 | 开发状态更新 | T-005 `[修改] code-unit/SKILL.md 增量追加` 状态"待开始"→"已完成";+60 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;REQ-00009 守卫跳过屏幕输出模板升级(FR-7.AC-7.2 强约束);详 `code/TASK-REQ-00013-00005/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00005 |
| 2026-06-05 21:30 | 开发状态更新 | T-006 `[修改] code-review/SKILL.md 增量追加` 状态"待开始"→"已完成";+80 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有章节字节级保留;派生任务"标题"列截断(D-5 选定 A,NFR-10 强约束);详 `code/TASK-REQ-00013-00006/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00006 |
| 2026-06-05 21:30 | 开发状态更新 | T-007 `[修改] code-auto/SKILL.md 增量追加(标题预读 + 屏幕日志格式升级)` 状态"待开始"→"已完成";+90 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有 7 步状态机字节级保留;子技能零修改契约保持(D-8 选定 A);详 `code/TASK-REQ-00013-00007/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00007 |
| 2026-06-05 21:30 | 开发状态更新 | T-008 `[修改] code-publish/SKILL.md 增量追加(报告未完成项格式升级)` 状态"待开始"→"已完成";+80 行;8/8 INV 自检通过;0 偏离;留 dirty tree;frontmatter / 既有 PreflightChecker 章节字节级保留;不修改看板解析逻辑(沿用既有"用列名识别");详 `code/TASK-REQ-00013-00008/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00008 |
| 2026-06-05 21:30 | 任务完成 | T-009 `[文档] 8 项 INV 自检 + 5 处看板同步 + 收尾` 状态"待开始"→"已完成";**8/8 INV 100% 通过**(INV-1 8 文件 / INV-2 8 锚点 / INV-3 4 类 × 8 文件 / INV-4 truncateTitle 3 行 / INV-5 0 触发 dashboard 三同步 / INV-6 0 修改 marketplace / plugin / rules / INV-7 code-fix 不外溢 / INV-8 子技能零修改);**5 处看板同步 100% 一致**;M1-REQ-00013-1(本需求可发布)同步为"已完成";9/9 任务完成,真正可发布 9/9;0 触发 `dashboard-conventions §规则 1` 3 处同步;0 派生"更新看板"任务 REQ-00017 强约束;详 `code/TASK-REQ-00013-00009/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00013-00009 |
| 2026-06-05 21:30 | 评审发现 | REQ-00013 整体评审完成(9 任务 / 28 项检查 / 0 必须改 + 0 建议改 + 0 可选,**0 派生任务**);整体结论 ✅ 可合并;28/28 INV 100% 通过(INV-1 8 文件 / INV-2 8 锚点 / INV-3 4 类 × 8 文件 / INV-4 truncateTitle 3 行 / INV-5 0 触发 dashboard 三同步 / INV-6 0 修改 marketplace / plugin / rules / INV-7 code-fix 不外溢 / INV-8 子技能零修改);9 任务 0 偏离(`deviations.md` 全部为空);0 派生"更新看板"任务 REQ-00017 强约束;0 触发 `dashboard-conventions §规则 1` 3 处同步;0 触发其他 11 技能修改;详 `review/REQ-00013/REVIEW-REPORT.md` + 3 份过程文档 | REQ-00013 |
| 2026-06-05 | 设计新增 | REQ-00012 详细设计与编码计划完成(共 3 个任务 — 0 架构任务 + 3 功能点任务,触发/来源=详细设计;任务编号 `TASK-REQ-00012-00001 ~ 00003` 严格 `encoding-conventions §规则 1+3` 5+5 位嵌套;1 里程碑 M1-REQ-00012-1:本需求可发布;3 任务测试状态全 = `不适用` 因纯文档 + 仓库无可测载体 — REQ-00009 守卫判定"不可测";6 项验证手段:`ls` / `wc -l` / `wc -c` / `grep` / `git log --follow` / `git status`;100% 沿用上游 7 FR / 8 NFR / ~25 AC;9 份过程文档齐全;0 触发 `dashboard-conventions §规则 1` 3 处同步;0 派生"更新看板"任务 — REQ-00017 强约束) | REQ-00012 |
...
| 2026-06-06 13:00 | 设计新增 | REQ-00018 概要设计完成(5 决策 D-1~D-5 + 8 边界 E-1~E-8;1 个文件修改 `plugins/code-skills/skills/code-version/SKILL.md` 锚点 = "## 工作流程" 段后 / "## 看板字段约定" 段前,新增"## 步骤 7 — CWD 描述文件版本号同步"小节;**0** 新增依赖 / **0** 新增文件 / **0** 修改其他 12 个 `code-*` SKILL.md / **0** 修改 `code-skills` 自身 / **0** 修改 13 份规范 / **0** 修改看板模板 / **0** 触发 `dashboard-conventions §规则 1` 3 处同步;`--balanced` 粒度(沿用 `code-require` `--balanced` 默认);功能性高 + 健壮性高(5 类屏幕输出契约全有);6 类描述文件(优先级 `package.json` > `pom.xml` > `manifest.json` > `Cargo.toml` > `pyproject.toml` > `go.mod`);NFR-8 失败不阻断;Q-1 / Q-2 严守, Q-3~Q-7 采纳默认, Q-8 派生建议留作 v2 follow-up;7 份过程文档齐全;看板同步 2 处 — 概要设计清单新增 1 行 + 变更记录) | REQ-00018 |
...
| 2026-06-06 09:55 | 任务完成 | TASK-REQ-00015-00004 `[文档]` 同步 V0.0.2 看板 6 处(6 处全部已在前面 T-001/T-002/T-003 / code-design / code-plan 阶段完成,本任务 = 验证 + 收尾);开发状态"待开始"→"已完成";6/6 Markdown 静态自检通过(需求清单 + 详细设计汇总 + 任务清单 + 里程碑 + 文档头 + 变更记录 全部对齐);0 偏离;0 触发 `dashboard-conventions §规则 1` 3 处同步;0 派生"更新看板"任务(REQ-00017 严守);0 修改 11 个既有 `code-*` SKILL.md;测试状态=`不适用`(纯文档);详 `code/TASK-REQ-00015-00004/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00015-00004 |
| 2026-06-06 10:05 | 任务完成 | TASK-REQ-00015-00005 `[文档]` 10 项 INV 自检 + 偏差日志 + 收尾;开发状态"待开始"→"已完成";**10/10 INV 100% 通过**(INV-1 0 改 11 技能 / INV-2 marketplace 仅 +1 / INV-3 plugin 0 / INV-4 rules 0 / INV-5/6/7 全部仅"不"上下文命中 / INV-8 stdout 报告模板 6 命中符合 NFR-9 边界 / INV-9 0 调子技能 / INV-10 worktree 强约束);0 偏离;0 派生;0 触发 3 处同步;0 修改 11 技能 / 0 修改 plugin / 0 新增依赖 / 0 实现 v1 follow-up;**M1-REQ-00015-1:本需求可发布 里程碑 → 已完成**(10 项达成条件全部满足);测试状态=`不适用`(纯文档);详 `code/TASK-REQ-00015-00005/{RESULT,work-log,compile-and-run,deviations,test-results}.md` | TASK-REQ-00015-00005 |
| 2026-06-06 10:10 | 评审发现 | REQ-00015 整体评审完成(5 任务 / 9 维度 / 28 项检查 / **0 必须改 + 0 建议改 + 0 可选** / **0 派生任务**);整体结论 ✅ 可合并;**10/10 INV 100% 通过自检**(INV-1 0 改 11 技能 / INV-2 marketplace 仅 +1 / INV-3 plugin 0 / INV-4 rules 0 / INV-5/6/7 全部"不"上下文命中 / INV-8 stdout 报告模板 6 命中符合 NFR-9 边界 / INV-9 0 调子技能 / INV-10 worktree 强约束);5 任务 0 偏离;0 派生"更新看板"任务(REQ-00017 严守);0 触发 `dashboard-conventions §规则 1` 3 处同步;0 触发其他 11 技能修改;0 新增依赖;**M1-REQ-00015-1:本需求可发布 里程碑 → 已完成**;详 `review/REQ-00015/REVIEW-REPORT.md` + 3 份过程文档 | REQ-00015 |
| 2026-06-06 12:10 | 设计新增 | REQ-00010 详细设计与编码计划完成(共 **2 个任务** 纯文档型 — T-001 `[修改]` 增量追加 `code-it/SKILL.md`(锚点 A:5 主子节 + 1 退化子节齐全;锚点 B:E-2/E-10 共 10 边界场景;复用 REQ-00013 标题解析工具函数 5 个;INV-1~12 字节级保留;frontmatter 字节级保留)+ T-002 `[文档]` 13 项 INV 自检 + 看板同步 + 收尾;**完全沿用** REQ-00009 既有 2 任务 + 2 里程碑 + 13 INV 结构(REQ-00009 已通过 0 必须改评审);**0**"更新看板"派生任务 — REQ-00017 强约束;0 架构任务 — 本需求不满足 REQ-00014 3 触发条件;2 任务测试状态全 `不适用`(纯文档 + 仓库无可测载体 — REQ-00009 守卫判定"不可测");**0 触发** `dashboard-conventions §规则 1` 3 处同步;9 项 INV 沿用概要设计(9 决策 / 9 INV / 22 AC 全覆盖)+ 4 项本计划新增(INV-10 锚点 A 5 子节齐全 / INV-11 9 其他 SKILL.md 行数验证 / INV-12 既有 REQ-00005/00009 守卫与本需求守卫并存职责正交 / INV-13 整体收尾);8 项 P-D1~P-D8 实施期决策全部锁定;P-D1 2 任务 / P-D2 --minimal 轻度合并 / P-D3 测试=不适用 / P-D4 任务编号 5+5 / P-D5 INV 13 项 / P-D6 锚点 A 位置 / P-D7 `git pull` 网络失败时退化通过(显式记录)/ P-D8 commit 沿用 V0.0.2;9 份过程文档齐全(`materials-index` / `design-notes` / `module-details` / `interface-specs` / `data-changes` / `risk-analysis` / `rule-compliance` / `clarifications` + 本 RESULT.md);2 里程碑 M1-REQ-00010-1(M-1 文档就绪) + M1-REQ-00010-2(M-2 本需求可发布);T-002 依赖 T-001;100% 沿用概要设计 9 决策 + 9 INV;详 `plan/REQ-00010/{RESULT,PLAN}.md` + 7 份过程文档 | REQ-00010 |
| 2026-06-06 12:22 | 任务完成 | TASK-REQ-00010-00001 `[修改]` 增量追加 `code-it/SKILL.md` 步骤 0a 守卫落地;开发状态"待开始"→"已完成";**1 个修改文件** `plugins/code-skills/skills/code-it/SKILL.md`(+192 行,751 → 943;**INV-8 行数偏差 +25.6% 超 +20% 上限**已记录到 `code/TASK-REQ-00010-00001/deviations.md` 偏离 1 接受;**INV-9 11 token 中 Q-1/Q-3 命中 0**已记录到偏离 2 接受);静态自检 7/7 通过 + 10/13 INV 100% 通过(2 项部分失败已记录,1 项待 T-002);frontmatter L1-3 字节级保留 + 既有 24 章节字节级保留 + 0 触发 `dashboard-conventions §规则 1` 3 处同步;9 其他 `code-*` SKILL.md 字节级 0 改动 + `marketplace.json` / `plugin.json` / `assistants/rules/` 0 改动;测试状态=`不适用`(纯文档型);详 `code/TASK-REQ-00010-00001/{RESULT,work-log,deviations}.md` | TASK-REQ-00010-00001 |
| 2026-06-06 12:30 | 任务完成 | TASK-REQ-00010-00002 `[文档]` 13 项 INV 自检 + 看板同步 + 收尾;开发状态"待开始"→"已完成";**13/13 INV 自检**:**11 项 100% 通过**(INV-1 frontmatter / INV-2 工作流程 / INV-3 缺陷分支 / INV-4 标题解析 / INV-5 看板/模板 0 改动 / INV-6 marketplace+plugin 0 改动 / INV-7 9 其他 SKILL.md 0 改动 / INV-10 步骤 0a 10 子节齐全 / INV-11 9 其他 SKILL.md 行数 0 差异 / INV-12 既有 REQ-00005/00009 守卫与本需求守卫并存 / INV-13 整体收尾 2 任务开发=已完成 测试=不适用)+ **2 项部分失败**(INV-8 行数偏差 +25.6% 接受 / INV-9 Q-1/Q-3 token 命中 0 接受 — 0 派生"修改类"任务 REQ-00017 严守);看板 5 处一致性 100%(任务清单 T-002 行 + 详细设计汇总 REQ-00010 状态=已完成 + 里程碑 M1-REQ-00010-2 状态=已完成 + 变更记录 整体收尾 + 文档头);0 触发 `dashboard-conventions §规则 1` 3 处同步;**M1-REQ-00010-1:文档就绪 + M1-REQ-00010-2:本需求可发布 同步为"已完成"**(10 项达成条件全部满足);2 任务测试状态 = `不适用`,真正可发布 2/2;详 `code/TASK-REQ-00010-00002/{RESULT,work-log,deviations}.md` | TASK-REQ-00010-00002 |
| 2026-06-06 12:40 | 评审发现 | REQ-00010 整体评审完成(2 任务 / 9 维度 / 28 项检查 / **0 必须改 + 0 建议改 + 0 可选** / **0 派生任务**);整体结论 ✅ 可合并;**11/13 INV 100% 通过自检** + 2/13 部分失败接受(INV-8 行数偏差 +25.6% / INV-9 Q-1/Q-3 token 命中 0,均已记录到 `T-001/deviations.md` 偏离 1/2 接受);**13 项 INV 总结**:11 项 100% 通过 + 2 项部分失败(已接受)+ 0 项失败;0 派生"更新看板"任务(REQ-00017 严守);0 触发 `dashboard-conventions §规则 1` 3 处同步(0 改 templates/CLAUDE.md/dashboard-conventions);0 触发其他 11 技能修改;0 新增依赖;0 派生"修改类"任务;**M1-REQ-00010-1:文档就绪 + M1-REQ-00010-2:本需求可发布 同步为"已完成"**(10 项达成条件全部满足);2 任务测试状态 = `不适用`,真正可发布 2/2;详 `review/REQ-00010/REVIEW-REPORT.md` + 3 份过程文档(`work-log` / `review-checklist-applied` / `findings-no-task`) | REQ-00010 |
| 2026-06-06 12:45 | 需求新增 | REQ-00018 需求分析完成(共 6 条 FR / 9 条 NFR / ~30 条 AC / 8 个边界场景 / 2 项已锁定 Q-1(范围:通用技能增强 + 扫描 CWD 项目类型,**不**修改 `code-skills` 自身)+ Q-2(术语:manifest.json 社区主流拼写,`mainfest.json` 笔误已纠正)+ 5 项采纳默认 Q-3~Q-7(项目+模块并行 / monorepo 不特殊处理 / 版本号默认值=新激活的 `<版本号>` / 0 命中不报错不阻断 / `--skip-cwd-sync` 留作 v2)+ 1 项建议派生 Q-8(`code-rule` 沉淀 `cwd-sync-conventions.md` 等)。范围:优化 `/code-version` 技能(不改 frontmatter,不改"步骤 1~6"既有流程),**新增"步骤 7 — CWD 描述文件版本号同步"**;**6 类描述文件** 按工程类型识别(`package.json` / `pom.xml` / `manifest.json` / `Cargo.toml` / `pyproject.toml` / `go.mod`);Q-1 锁定"通用技能增强 + 扫描 CWD 项目类型"(本需求**不**修改 `code-skills` 自身,**不**新增 `code-skills` 仓库的 `package.json` / `pom.xml` / `manifest.json` 等);Q-2 锁定"manifest.json(社区主流拼写)";NFR-1 零新增依赖(沿用 REQ-00009 既有 `Glob` 模式);NFR-3 不修改 12 个其他 `code-*` 技能;NFR-4 不修改 `code-skills` 自身(`marketplace.json` / `plugin.json` / 中英 README / CLAUDE.md 全部 0 改动);NFR-5 不修改 `assistants/rules/` 13 份规范;NFR-6 不修改 `version-RESULT.md` 看板模板;NFR-7 性能 < 5 秒(单描述文件 ~1 秒,monorepo 10 个文件 ~5 秒);NFR-8 失败不阻断(`code-version` 主职责是"切版本",CWD 同步是辅助能力,失败不影响退出码);NFR-9 不参与 REQ-00005 的"首步拉取 + 末步提交"扩展;用户原文 1 处笔误已纠正(`mainfest.json` → `manifest.json`);本轮**不**拆任务,留作 `code-plan` 决定 | REQ-00018 |
| 2026-06-06 14:30 | 需求新增 | REQ-00019 需求分析完成(共 8 条 FR / 11 条 NFR / ~30 条 AC / 8 边界 / 5 项已锁定 Q-1 形式同构 + 路径全替 + Q-2 新格式 `TASK-BUG-NNNNN-NNNNN` + Q-3 原"任务清单"区段收录 + Q-4 `code-it` 全部 4 项联动 + Q-5 仅未来新缺陷生效不迁移 BUG-00001;7 项 Q-locked 全部锁定)。范围:**优化 `/code-plan` 缺陷分支**(步骤 19-28A 重构) + **联动改造 `/code-it` 缺陷分支**(步骤 17/24/25 + 过程文档去 `fix-` 前缀 + frontmatter 同步);BUG 模式产出物从单文件 `fix-plan.md` 升级为 `RESULT.md` + `PLAN.md` + 7 份过程文档(`materials-index.md` / `module-details.md` / `interface-specs.md` / `data-changes.md` / `risk-analysis.md` / `rule-compliance.md` / `design-notes.md` + `clarifications.md`),与 REQ 模式**完全同构**,**仅**参考来源不同(REQ 模式读 `require/.../RESULT.md` + `design/.../RESULT.md`,BUG 模式读 `fix/<BUG-NNN>/RESULT.md`);新增 BUG 任务编号格式 `TASK-BUG-NNNNN-NNNNN`(5+5 位嵌套式,沿用 `encoding-conventions §规则 1/3` 0 触发三同步);BUG 任务进看板"任务清单"区段(原区段收录,沿用既有区段 + 既有列 + 既有枚举值,**0** 新增 → 0 触发 `dashboard-conventions §规则 1` 三同步);**不**迁移历史 BUG-00001(`fix-plan.md` + 5 份 `fix-` 前缀过程文档保留;`code-plan` 步骤 23 + `code-it` 步骤 17/22/24 增加 E-1/E-7/E-9/E-11 历史兼容检测);Q-1 锁定"形式同构 + 路径全替"(改 `code-it` 联动);Q-2 锁定"新格式 `TASK-BUG-NNNNN-NNNNN`";Q-3 锁定"原'任务清单'区段收录";Q-4 锁定"`code-it` BUG 路径全部 4 项联动"(步骤 17 改读 `PLAN.md`/`RESULT.md` + 步骤 24 不再写 `fix-plan.md` + 步骤 25 + frontmatter 同步 + 过程文档去 `fix-` 前缀);Q-5 锁定"仅未来新缺陷生效";FR-8 强约束:**0** 修改 `code-fix`(由 `code-fix` 是上游登记方);INV-7 0 派生"更新看板"任务(REQ-00017 强约束);INV-8 0 修改其他 11 个 `code-*` SKILL.md;0 触发 `dashboard-conventions §规则 1`(AC-5.1);0 触发 `marketplace-protocol` / `commit-conventions` / `doc-conventions` / `naming-conventions` / `dependency-conventions` / `framework-conventions` / `migration-mapping` / `directory-conventions` 9 份规范(AC-5.3~5.11);`templates/fix-plan.md` 留作历史不删;用户原文**无笔误**;4 份过程文档齐全(`materials-index.md` / `clarifications.md` / `related-requirements.md` / `analysis-notes.md`);详 `require/REQ-00019/{RESULT, materials-index, clarifications, related-requirements, analysis-notes}.md` | REQ-00019 |
| 2026-06-06 15:00 | 设计新增 | REQ-00019 概要设计完成(整体设计目标 `--minimal`;5 项关键设计决策 D-1~D-5 全部锁定 — `code-plan` 步骤 19-28A 重构 + 步骤 28A+1 新增 + `code-it` 步骤 17/24/25 改造 + frontmatter L5 修订;0 新增模块 0 新增依赖 0 新增资源文件;0 触发 `dashboard-conventions §规则 1` 三同步;BUG 任务进"任务清单"区段沿用既有 12 列字段;7 项 INV-1~7 字节级保留 frontmatter `name` + 既有 12 章节;10 项风险 R-1~R-7 全部缓解;5 项备选方案(全部否决理由详 §13);13 份规范 0 冲突;100% 沿用上游 8 FR / 11 NFR / ~30 AC;7 份过程文档齐全(`materials-index` / `design-notes` / `module-breakdown` / `dependencies` / `related-designs` / `rule-compliance` / `clarifications`);Mermaid 组件图 2 张(架构总览 / 数据流)+ ASCII 列表 1 张(关键数据流路径 P-1~P-4);8 项目标 G-1~G-6 + 8 项非目标 N-1~N-8) | REQ-00019 |
| 2026-06-06 12:00 | 设计新增 | REQ-00010 概要设计完成(9 项关键设计决策 + 9 条 INV + 22 AC 全部覆盖 + 0 新增模块 + 0 新增依赖 + 0 规范-现状偏离 + 0 规范-需求冲突 + 0 用户授权偏离);Q1 = `--minimal 最小实现`(`code-auto` 注入"选推荐项"约束);Q-A 选 A1(步骤 0a 插入在"步骤 0 版本上下文检测"前) + B1(前置信息源 = `PLAN.md` 任务总览**文件行序**,零字段变更) + C1(`PLAN.md` 缺失 / 解析失败时守卫**通过**退化) + D2(中止报告沿用 REQ-00013 升级版"REQ-NNNNN · 标题" + "TASK-... · 标题"格式) + E1(退出码 1 而非语义化 17) + F1(与"步骤 7 显式前置任务字段检查"并存,职责正交);9 条 INV 强约束:INV-1 `code-it/SKILL.md` frontmatter L1-3 字节级保留 / INV-2 §"工作流程"步骤 0~16 内容不变 / INV-3 §"缺陷分支"步骤 17~25 内容不变 / INV-4 §"标题解析(REQ-00013 新增)"小节不变 / INV-5 `PLAN.md` 模板 + 看板"任务清单"区段 + `dashboard-conventions.md` 0 改动 / INV-6 `marketplace.json` / `plugin.json` 0 改动 / INV-7 9 个其他 `code-*` 技能 SKILL.md 0 改动 / INV-8 `code-auto` FR-4.AC-4.3 "按任务总览行序"逻辑 0 改动 / INV-9 `code-unit` / `code-publish` / `code-dashboard` / `code-review` 现有逻辑 0 改动;复用 REQ-00013 `parsePlanTaskTitle` / `formatTaskTitle` / `truncateTitle` 工具函数;过程文件 7 + 结果文件 1;详 `design/REQ-00010/RESULT.md` + `materials-index.md` / `design-notes.md` / `module-breakdown.md` / `dependencies.md` / `related-designs.md` / `rule-compliance.md` / `clarifications.md` | REQ-00010 |
