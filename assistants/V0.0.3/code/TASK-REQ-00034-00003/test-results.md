...
| 校验点 | 验证方式 | 预期 | 实际 |
| --- | --- | --- | --- |
| `code-plan/SKILL.md` 4 处 `code-unit` 字面改写 | `git diff` | 全部改写为"`code-it` 步骤 8.5 内化" | ✓ |
| `code-plan/SKILL.md` frontmatter L1-3 字节级保留 | `cat L1-3` | 字节级一致 | ✓ |
| 任务"测试状态"2 枚举(`已运行-通过` / `不适用`)**0**改 | `git diff` | 0 改 | ✓ |
| 任务"双状态"语义 = 开发状态 = 已完成**0**改 | `git diff` | 0 改 | ✓ |
| 净变化 -10~+20 | `git diff --stat` | +4/-4 | ✓ |
