...
## 16. 验证结果(本节为 T-006 占位,等 REQ-00031+ 落地后填写)

| 验证项 | 阈值 | 实际(待 3 个新 REQ 落地) | 验证时间 | 通过/失败 |
| --- | --- | --- | --- | --- |
| AC-8.1:3 个新 REQ `design/.../RESULT.md` 平均行数 | ≤ 184 | (待 REQ-00031+ 落地后填写) | (待观测) | (待观测) |
| AC-8.2:3 个新 REQ `plan/.../RESULT.md` 平均行数 | ≥ 340 | (待 REQ-00031+ 落地后填写) | (待观测) | (待观测) |

**观测方法**(用户人工执行):
1. 收集 3 个新 REQ 的 `design/.../RESULT.md` 行数(用 `wc -l`)
2. 收集 3 个新 REQ 的 `plan/.../RESULT.md` 行数(用 `wc -l`)
3. 计算平均,与 AC-8 阈值对比
