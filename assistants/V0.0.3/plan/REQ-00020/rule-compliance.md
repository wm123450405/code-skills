...
## 7. 任务粒度调整规则(本需求 FR-3 +3 行)对规范的符合性

- **0 触发** `dashboard-conventions §规则 1`:任务粒度表 +3 行**不**新增"任务清单"区段列/枚举/区段
- **0 触发** `encoding-conventions §规则 1/3`:沿用既有任务编号体系
- **0 触发** `commit-conventions`:0 派生"更新看板"任务(REQ-00017 强约束)
- **0 触发** `naming-conventions`:0 新增文件名前缀
