...
- **无显著偏离**:本设计不修改项目结构 / 模块边界 / 依赖方向;仅在 5 主流程技能 SKILL.md 中追加 1 个新步骤(在既有步骤 0 之后,工作流程之前)。
- 自检:每个 SKILL.md 改写严格遵循"锚点"约定(`skill-conventions.md` 沿用:`## 工具使用约定` 段后 + `## 工作流程` 段前),**不**修改 frontmatter / 既有章节。
