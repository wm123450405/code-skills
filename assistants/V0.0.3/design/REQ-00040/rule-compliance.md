...
| 规范条款 | 现状 | 本设计是否偏离 |
| --- | --- | --- |
| `dashboard-conventions §规则 1`(看板字段扩展三同步) | 当前看板 7 列(缺陷编号/严重度/标题/状态/报告时间/修复人/关联需求) | **不偏离** — 本设计**不**新增看板列(产物放 `fix/<BUG-NNN>/reproduce/` 子目录,**不**进总览 / 看板) |
| `skill-conventions §规则 1`(SKILL.md frontmatter name + description) | `code-fix/SKILL.md` frontmatter 已含 | **不偏离** — 本设计**不**改 frontmatter(L1-3 字节级保留) |
...
| `directory-conventions`(目录与模块规范) | §规则 1 占位待添加;既有 `code-fix/templates/` 布局 | **不偏离** — 本设计**不**新增项目级目录,仅在 `fix/<BUG-NNN>/` 内新增运行时子目录 `reproduce/`(沿用 `assistants-layout.md` 既有"fix/BUG-NNN/ 下子目录"约定) |
...
| `module-conventions §规则 1` | ⚠️ 软(DEPRECATED) | 本设计**不**新增 `code-fix` 子目录(沿用既有 `templates/`);`reproduce/` 是运行时子目录 |
| `directory-conventions` | ⚠️ 软(规则 1 占位) | 本设计**不**新增项目级目录 |
| `dashboard-conventions §规则 1` | ❌ 0 触发 | 本设计**不**新增看板列(产物放子目录) |
| `encoding-conventions §规则 1-4` | ❌ 0 触发 | 本设计**不**新增编码格式 |
| `coding-style.md` | ❌ 0 触发 | SKILL.md 是自然语言,无代码风格约束 |
| `commit-conventions.md` | ⚠️ 软(规则 1 占位) | 沿用既有 `chore(code-it):` / `chore(code-design):` / `chore(code-plan):` 前缀;**不**新增规则 |
| `dependency-conventions.md` | ❌ 0 触发 | 本设计**0 新增三方依赖**;playwright/puppeteer/headless-chrome 仅运行时探测 |
| `framework-conventions.md` | ❌ 0 触发 | 本设计**不**涉及项目架构 |
| `naming-conventions.md` | ⚠️ 软(规则 1 占位) | `reproduce/` 命名遵循"小写 + 复数"风格(沿用 `fix/` / `code/` / `plan/` 等既有目录命名) |
| `doc-conventions.md` | ❌ 0 触发 | 本设计**不**涉及 README |
| `marketplace-protocol.md` | ❌ 0 触发 | 本设计**不**涉及插件清单 |
| `migration-mapping.md` | ❌ 0 触发 | 本设计**不**涉及编码迁移 |

**自检结论**:0 违反强约束(`skill-conventions §规则 1` / `§规则 2`),0 触发 `dashboard-conventions §规则 1` 三同步,0 新增 `code-rule` 介入。
