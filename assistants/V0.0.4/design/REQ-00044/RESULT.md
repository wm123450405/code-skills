...
| 旧技能 | 处理方式 |
| --- | --- |
| `code-require` | 保留 SKILL.md,frontmatter 追加 `(DEPRECATED: 使用 code-req)` |
| `code-design` | 同上 |
| `code-plan` | 同上 |
| `code-it` | 同上 |
| `code-check` | 同上 |
...
> **决策 D-1**:旧技能 SKILL.md 保留而非删除,原因:① 历史版本(V0.0.0~V0.0.4)的 `assistants/` 中的引用仍指向旧技能名;② 便于用户过渡期查阅旧逻辑;③ 在 marketplace 发布时,新 `plugin.json` 只列出 7 个新技能,旧技能不会被注册但文件保留。
