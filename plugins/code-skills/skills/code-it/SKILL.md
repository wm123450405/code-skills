...
本技能是本技能库中**唯一**被允许修改 `<本仓库>` 中除了 `./assistants` 目录中的其他代码文件的技能。`code-require` / `code-design` / `code-plan` / `code-fix` 不得修改这些工程代码。本技能读 `<版本号>/code/<TASK-...>/` 工作空间文档与 `plan/<REQ>/PLAN.md` 任务清单,逐任务产出 `code/<TASK-...>/RESULT.md` 实施记录。
...
> 本节是 `code-it` 启动时执行"过程文档生成判定"的依据。AI 在执行过程中按本节的"不涉及/不适用"判定准则自适应决定本轮是否生成对应过程文档(任务级)。
...
| `unit-test-results.md` | 沿用 REQ-00034 步骤 8.5(按需写单测);函数级代码类任务 → 写;文档/配置/类型定义 → 占位 |
| 看板"变更记录" | **本轮有追加 → 生成;无变更 / 幂等空跑 → 不追加** |

### 执行流程

1. 步骤 0(版本检测)完成后,执行本步骤
2. 读上游输入(PLAN.md 任务详情 / 详细设计 / 项目级规范)
3. 对每类过程文档,按"适用过程文档清单(任务级)"判定
...
**审查改修任务**(`触发/来源` = `审查改修`,由 `code-check` 派生):
...
```
1. 屏幕输出"⏭ code-it 跳过单测(项目不可测)"
2. 输出守卫检查详情(具体哪些文件命中/未命中)
3. **不**调用任何 `Bash`(本步骤不写单测 + 不跑单测)
4. **不**写 `code/<任务编码>/unit-test-results.md`(但仍写 `code/<任务编码>/test-results.md` 作为"项目自身的集成/冒烟测试"占位)
5. **不**写 `work-log.md` / `compile-and-run.md` / `deviations.md`(本步骤不涉及)
6. 写看板"任务清单"区段:本任务行"测试状态"列 = `不适用`
...
| 守卫不通过 + 跳过 | 0 | 继续下一个步骤 |
| 守卫通过 + 进入步骤 8.5 | 0 | 继续 |
| 守卫通过 + 步骤 8.5 写单测跑通 | 0 | 继续 |
| 守卫通过 + 步骤 8.5 写单测跑通失败 | ≠ 0 | 中断(进入步骤 12 错误修复循环) |
| 守卫通过 + 步骤 8.5 崩溃 | ≠ 0 | 中断 |
| 任务编码不存在 / 无 `.current-version` | ≠ 0 | 中断(原行为) |
...
任务:<任务编码>
判定:写单测 / 跳过单测
测试框架:<Jest / Pytest / Go test / JUnit / Vitest / ...>
新增/修改的测试文件:<file1.test.ts, file2.test.ts>(若有)
跑通情况:<通过 N 个 / 失败 M 个>(若写了单测)
```
...
- 跳过验证直接进入## 步骤 9
- 在错误未消除的情况下标记任务为"已完成"
- 用 `--testPathIgnorePatterns` / 跳过失败用例 / 注释掉测试 等绕过手段
...
| 场景 | 退出码 | `code-auto` 响应 |
| --- | --- | --- |
| 守卫不通过 + 跳过(沿用 8a.3) | 0 | 继续 |
| 守卫通过 + 写单测跑通 | 0 | 继续 |
| 守卫通过 + 写单测跑通失败 + 5 次停下 | ≠ 0 | 中断(询问用户) |
| 守卫通过 + 写单测跑通失败 + 用户继续 | 0 | 继续 |
| 守卫通过 + 崩溃 | ≠ 0 | 中断 |
| 任务编码不存在 / 无 `.current-version` | ≠ 0 | 中断(原行为) |
...
### 步骤 8.7 — 过程文档自适应判定执行(由 code-it 内化,BUG-00004 新增)

> 适用对象:本技能在"## 步骤 8.6 逻辑行统计"**之后**、"## 步骤 9 编译验证"**之前**的**过程文档决策**收尾。**仅**在 `code-it` 步骤 1 判定为任务分支(任务编码匹配 `^TASK-REQ-\d{5}-\d{5}$` 5+5 位嵌套式)时触达;缺陷分支(`TASK-BUG-...`)**仍触达**(本需求为修复 BUG-00004,缺陷分支必须执行)。
> 依据:BUG-00004 详细设计 §5 算法 1 + §5 算法 2 + §4 模块 1;原"## 过程文档自适应判定"(line 101-138)定义判定准则,本步骤把判定结果**物化**为 `decisions` 字典。
> 触发条件:**仅**在步骤 0a 守卫**通过**或缺陷分支判定为 `TASK-BUG-...` 时执行。

#### 步骤 8.7.1 目标

把"## 过程文档自适应判定"小节定义的判定准则**物化**为可执行的 `decisions` 字典,作为步骤 9 / 10 / 11 / 13 / 16 的输入,避免"判定表与工作流脱节"问题(沿用 BUG-00004 根因)。
...
function applyProcessDocDecisions(taskNum, taskType, changedFiles, testable):
 ...
 # 2. compileAndRun.md 判定(纯文档/纯配置 → 不生成)
 if any(file endsWith .md or .json or .toml or .yaml or file in {*.d.ts, *.config.*} for file in changedFiles):
 ...
 # 函数级代码类任务 → 写单测 + unit-test-results.md
 ...
 # 7. process-doc-decisions.md 判定(本文件自身)
...
1. **判定时机**:步骤 8 末尾(步骤 8.6 之后),**不**在步骤 0 之前 — 原因:步骤 8 末尾已知 `testable` + `taskType` + `changedFiles`,信息完备
2. **判定方式**:沿用既有 `## 过程文档自适应判定` 表格的"判定准则"列,代码化即可,**不**改判定准则
3. **`process-doc-decisions.md` 自指**:本文件本身也算"过程文档";若其他都是"生成",本文件**不**生成(避免自身指涉)

#### 步骤 8.7.4 边界条件(NFR-2 / NFR-7)

| E 边界 | 触发场景 | 处理 |
| --- | --- | --- |
| **E-1** | `decisions` 字段缺失(防御性) | 视为"按原行为执行"(沿用 NFR-4 幂等) |
| **E-2** | 任务类型字段缺失 | 退化:按 `修改` 判定 |
| **E-3** | `changedFiles` 为空 | 退化:按"非纯文档"判定(避免漏生成) |
| **E-4** | 步骤 8a 守卫未执行(缺陷分支) | 退化:`testable = False` → `unitTestResults = '不生成'` |
| **E-5** | `process-doc-decisions.md` 写入失败 | 屏显 `⚠`,不阻断(沿用既有) |
| **E-6** | `code-auto` 上下文 | 步骤 8.7 仍执行(沿用既有) |

#### 步骤 8.7.5 性能(NFR-1)

- **总耗时**:**< 0.1 秒**(纯 O(1) 判定,无 IO)
- **跳过策略**:N/A
...
**成功**:
```
=== code-it 过程文档自适应判定(步骤 8.7)===
任务:<taskNum>
判定:
 - workLog.md: 生成
 - compileAndRun.md: 生成 / 不生成
 - deviations.md: 生成
 - testResults.md: 生成 / 不适用
 - unit-test-results.md: 生成 / 不生成
 - 看板"变更记录": 生成 / 不追加
 - process-doc-decisions.md: 生成 / 不生成
```

**失败**(防御性):屏显 `⚠ code-it 过程文档判定跳过(<原因>),不阻断本任务`
...
| --- | --- | --- |
| `decisions` 物化成功 | 0 | 继续 |
| `decisions` 物化失败(防御性) | 0 | 继续(NFR-7 不阻断) |
| 任务编码不存在 / 无 `.current-version` | ≠ 0 | 中断(原行为) |

#### 步骤 8.7.8 不变量

- **不**修改 frontmatter(L1-3 字节级保留,NFR-7)
- **不**修改既有"## 过程文档自适应判定" 章节(只引用,**不**改判定准则)
- **不**修改"## 步骤 8 / 8.5 / 8.6" 既有逻辑(只在末尾追加)
- **不**触发 `AskUserQuestion`(NFR-3 强约束)
- **不**新增 CLI 参数(NFR-3 强约束)
- **不**触发 `dashboard-conventions §规则 1` 三同步(本步骤**不**改看板字段)

### 步骤 9 — 编译验证
> **本步骤守卫(步骤 8.7)**:**仅**当 `decisions.compileAndRun == '不生成'` → 本步骤跳过(直接到步骤 12);其余情况执行既有 5 步逻辑(字节级保留)。
> 退化:若 `decisions` 字段缺失(防御性,E-1)→ 视为"按原行为执行"(沿用 NFR-4 幂等)。
1. **检测构建命令**
2. **执行构建**:`Bash` 执行
...
### 步骤 10 — 启动运行验证
> **本步骤守卫(步骤 8.7)**:**仅**当 `decisions.compileAndRun == '不生成'` → 本步骤跳过(直接到步骤 12);其余情况执行既有 6 步逻辑(字节级保留)。
> 退化:同步骤 9。
1. **检测运行命令**
2. **执行**:`Bash` 运行,设置超时
3. **验证启动成功**
...
### 步骤 11 — 测试(若适用)
> **本步骤守卫(步骤 8.7)**:**仅**当 `decisions.testResults == '不生成'` → 本步骤跳过(直接到步骤 12);其余情况执行既有 6 步逻辑(字节级保留)。
> 退化:同步骤 9。
1. **检测测试命令**
2. **执行**:`Bash` 运行
...
> **本步骤产物 `RESULT.md` 末尾追加 "## 8. 过程文档清单(由 code-it 内化,BUG-00004 新增)" 区段**(沿用 BUG-00004 详细设计 §5 算法 3 `write_process_doc_list` 伪代码 + §模板 §8 渲染逻辑)。
> 该段含:
> - **§8.1 工作流上下文**:`decisions` 字典的对象字面
> - **§8.2 决策结果表**:7 类过程文档 × `生成` / `不生成` + 判定理由
> - **§8.3 决策依据**:引用 `## 过程文档自适应判定`(line 101-138)与步骤 8.7(line 805+)的判定准则
> - **§8.4 关联任务**:沿用模板原"## 8. 关联任务"段(4 行:前置任务 / 后置任务 / 取代任务 / 关联 code-check)
>
> **不变量**:
> - 既有 "## 7. 已知问题 / 未完成项" 段字节级保留
> - "## 9. 单元测试" / "## 10. 逻辑行统计" / "## 11. 变更记录" 段**标题字面保留**(不偏移 +1)
> - 模板原"## 8. 关联任务" 4 行内容**字面保留**(作为 §8.4 子节)
> - `decisions` 缺失(防御性)→ 退化:按既有写法(全 `✅`,沿用 NFR-4 幂等)
>
> 触发条件:**仅**在 `code-it` 步骤 8.7 物化 `decisions` 成功时执行;步骤 8.7 失败(E-5 `process-doc-decisions.md` 写入失败)→ §8 段**不**追加(避免渲染不一致)。
...
> **本步骤产物追加 "## 已生成的过程文档清单" + "## 已跳过(不生成)的过程文档清单" 2 段**(沿用 BUG-00004 详细设计 §5 算法 3 末段 + §模板 §8.3 渲染逻辑)。
> 该 2 段含:
> - **§已生成**:本任务实际写入的文件清单(如 `work-log.md` / `deviations.md` / `RESULT.md`;若步骤 9/10/11 守卫未触发 + 测试状态≠不适用 → 加 `compile-and-run.md` / `test-results.md`)
> - **§已跳过**:本任务判定为"不生成"的文件清单(如 `compile-and-run.md`(纯 Markdown 改造)+ `test-results.md`(测试状态=不适用)+ `unit-test-results.md`(项目不可测))
> - **§引用源**:`decisions` 字典来源(步骤 8.7 物化)
>
> **不变量**:
> - 既有 5 条汇报要点(改动文件 / 编译启动测试结果 / 关键决策与偏离 / 版本看板同步 / 下一步建议)字节级保留
> - 汇报顺序保持(`先文件 → 再结果 → 再决策 → 再同步 → 再建议`)
>
> 触发条件:**仅**在 `code-it` 步骤 8.7 物化 `decisions` 成功时执行;步骤 8.7 失败 → 汇报不附加该 2 段(沿用既有 5 条汇报)。
...
> - 过程文档以 `TASK-BUG-` 任务编号命名(`work-log.md` / `compile-and-run.md` / `test-results.md` / `deviations.md`,沿用任务路径同套命名);原 `fix-` 前缀退场(本需求后**不**再使用;`` 等留作历史不删,见 E-9 边界)
> - 实施完成时同步推进缺陷状态
...
> 本边界是 BUG-00001 历史 `` 的兼容检测(仅 BUG-00001 受影响;本需求后**不**再生成 ``)。
...
3. 存在 → 复用(若之前 `code-it` 跑过中断,继续基于 `` 等恢复)
...
**过程记录**:
- 持续追加到 `code/<TASK-BUG-...>/work-log.md`(每完成一个文件、每跑一次命令、每遇到一个问题都记;原 `` 退场,见 E-9 边界)
- 重要的临时决策、踩坑、查到的关键信息都记
- 修复过程中发现的"非本 bug"问题 → 记到 `code/<TASK-BUG-...>/deviations.md`,**不擅自修复**

#### E-9 边界:`` 存在

> 本边界是 BUG-00001 历史 `` / `` / `` 的兼容检测(仅 BUG-00001 受影响;本需求后**不**再生成 `fix-` 前缀过程文档)。
...
> - 测试用例来源:缺陷分支读 `PLAN.md` 中"测试方案"列出的用例(原 `` 退场,沿用既有读取路径)
...
> 本边界是 BUG-00001 历史 `` 在步骤 24 不再写动作的兼容检测。
...
- 不读 `PLAN.md` / `` / `RESULT.md` / `rules/` 就直接动手
