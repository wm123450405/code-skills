...
- 守卫**不**触达(由原 §"步骤 0 — 版本上下文检测" 处理,提示调 `code-version`)
...
#### E-4:当前任务不在 `PLAN.md` 任务总览中
- 视为"任务编码不存在",由原 §"步骤 2 — 校验任务存在" 报错
- 守卫不重复(职责正交)
...
#### E-8:任务编码格式不匹配(`TASK-...` / `REQ-...-...`)
- 由原 §"步骤 1 — 收集并解析输入 ID" 报错"任务编码格式不匹配"
- 守卫**不**触达

#### E-9:任务属于缺陷分支(`TASK-BUG-...`)
- 守卫**不**触达(缺陷分支无 `PLAN.md`,走 §"缺陷分支" 17-25 流程)
- `if not taskNum.startsWith('TASK-REQ-'): return` 提前退出
