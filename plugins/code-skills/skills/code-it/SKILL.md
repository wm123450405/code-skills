...
本技能是本技能库中**唯一**被允许修改 `<本仓库>/skills/*/SKILL.md` 的技能。`code-require` / `code-design` / `code-plan` / `code-fix` 不得修改这些工程代码;`code-unit` 不得修改生产代码(只能写测试代码)。本技能读 `<版本号>/code/<TASK-...>/` 工作空间文档与 `plan/<REQ>/PLAN.md` 任务清单,逐任务产出 `code/<TASK-...>/RESULT.md` 实施记录。
