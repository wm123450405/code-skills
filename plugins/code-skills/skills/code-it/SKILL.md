...
> **本技能职责范围**(本需求 REQ-00031 FR-4 + 本需求 REQ-00034 FR-2/FR-3 声明,2026-06-12 + 2026-06-15 起生效):本技能的职责是"编码 + 编译/运行成功 + **按需写单测**(项目可测性守卫 7 项 + 写单测 + 跑通);**不**弹 `AskUserQuestion`(沿用原 `code-unit` 步骤 0a 守卫逻辑,字节级沿用)。`code-it` 步骤 9-12 中的"测试"语义**收窄为"项目自身的集成/冒烟测试"**(可启动运行的检查),与"单元测试"无关。

> **本需求 REQ-00034 FR-2/FR-3 改造**(2026-06-15 起生效,沿用原 `code-unit` 步骤 0a + 步骤 7 字节级):
...
### 步骤 8a — 项目可测性守卫(本需求 REQ-00034 FR-2 新增,2026-06-15 起生效)
...
### 步骤 8.5 — 按需写单测(本需求 REQ-00034 FR-3 新增,2026-06-15 起生效)
...
> 依据:本需求 REQ-00034 FR-3;原 `code-unit` 步骤 4 / 7 / 8 / 9 字节级保留(Q-4 隐含答复 C)。
...
任务:<任务编码>
判定:写单测 / 跳过单测
测试框架:<Jest / Pytest / Go test / JUnit / Vitest / ...>(沿用原 code-unit 步骤 8 检测)
...
最多连续失败 5 次后必须停下询问用户(沿用原 code-unit 步骤 11 + 本需求 E-8 锁定)
```
