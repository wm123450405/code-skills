...
**双状态语义**(本需求 REQ-00031 FR-3 简化,2026-06-12 起生效):**任务"真正可发布" = 开发状态=已完成**。本字段在 V0.0.2 期间用于"任务自含单元测试"语义;V0.0.3 修订后,`code-plan` **不**规划测试任务,本字段由 `code-it` 初始填 `不适用`;若项目需要单元测试,在 `code-unit` 阶段另起流程记录到 `code/<任务>/test-results.md`(与 `code-plan` 任务粒度**正交**)。
