...
└── <版本号>/
    ...
    ├── req/<需求编码>/ # code-req 产出(本技能只读)
    └── fix/<缺陷编码>/ # code-fix 产出(本技能只读)
```

- 路径以**当前工作目录(CWD)**为基准
- 本技能**不修改** `./assistants/rules/` 下任何文件
- 本技能**不修改** `<版本号>/RESULT.md`(FR-6 仅**读** + **打印**自检报告,**不修改**任何字段)
- 本技能**不修改** 7 个既有 `code-*` SKILL.md
- 本技能**不**写 `assistants/<版本号>/req/` 或 `fix/` 下的任何文件(本技能无任务编码概念)
...
- `assistants/V<版本>/req/REQ-*/REQUIRE.md`
- `assistants/V<版本>/req/REQ-*/DESIGN.md`
- `assistants/V<版本>/req/REQ-*/PLAN.md`
- `assistants/V<版本>/req/REQ-*/PROCESS.md`
- `assistants/V<版本>/fix/BUG-*/BUG.md`
- `assistants/V<版本>/fix/BUG-*/PLAN.md`
- `assistants/V<版本>/fix/BUG-*/PROCESS.md`
...
- 自检结果**不**影响退出码(NFR-2)
- 复用简化版 2 区段,**不**新增区段 → **不**触发 `dashboard-conventions §规则 1` 同步(REQ-00013 INV-5 协同)
...
- **REQ-00010**(`/code-req`):本技能不调 `code-req`(本技能无任务编码概念,纯 CLI 操作)
...
- 不要修改 7 个既有 `code-*` SKILL.md(INV-1 严守)
...
| 时间 | 版本 | 变更摘要 | 变更人 |
| --- | --- | --- | --- |
| 2026-06-06 09:20 | v1 | 初始创建:8 FR + 10 NFR + 12 边界 E-M1~M12 + 10 项 INV 严守。范围:新增第 7 个 `code-*` 技能 `code-merge`,在 git worktree 模式下自动执行 FR-1 → FR-2 → FR-3 → FR-4 → FR-5 → FR-6 → FR-7 → FR-8,含 LLM 智能合并冲突解决(看板数据保留双方 + 按时间戳排序 + 统计行重新计算;代码 / 配置 / 文档 LLM 智能合并;二进制留 unmerged) + 看板 2 区段自检(复用既有字段不修复) + `git merge --no-ff` 合回 main + **不**产生过程/结果文件(SKILL.md 必产) + **不**自动 push / **不**自动清理 worktree。`code-req --auto` 完成后**不**自动调本技能(职责分离) | wangmiao |
| 2026-06-30 17:00 | v2 | 适配新结构:更新路径引用(`require/`/`design/`/`plan/`/`code/`/`review/` → `req/`/`fix/`),更新技能名引用(code-version→code-ver, code-auto→code-req, code-publish→code-ver --publish),看板自检从 5 区段简化为 2 区段 | wangmiao |
