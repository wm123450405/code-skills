---
name: code-auto
description: 一键跑通完整开发周期。给一段需求(或需求编号/缺陷编号),本技能自动按"需求分析 → 概要设计 → 详细计划 → 编码(含单测) → 评审 → 自动修复必须改"的固定顺序串行驱动 5 个子技能,中间所有问询都自动选推荐项,全程无需人工确认;支持 Ctrl+C 中止,完成时输出报告。支持"已登记的需求/缺陷"续跑。也可用作"从需求到代码 + 单测 + 评审全自动跑通"的一键命令。
---
...
| 步骤 | 子技能 | 输入参数 | 期望产物 | 失败处理 |
| --- | --- | --- | --- | --- |
| 1 | `code-plan` | `<BUG-NNN>` | `fix/<BUG-NNN>/` | 中断 + 报告(1) |
| 2 | `code-it` | `<BUG-NNN>` | `fix/<BUG-NNN>/` 等 | 中断 + 报告(1) |
...
### 步骤 1:code-require(条件化,改造:沿用路径感知判定)
...
- **期望产物**:`./assistants/<版本号>/fix/<BUG-NNN>/`
- **失败处理**:子技能退出码 ≠ 0 → 中断 + 报告(退出 1)
...
| **E-7** | 评审循环无收敛 | 持续循环;用户可 `Ctrl+C` | FR-5.AC-5.5 |
...
| **E-15** | 模式 B 缺 `RESULT.md` | — | 沿用 `req-run-require` 模式自动降级(目录存在 + RESULT.md 不存在 → 视为"未完成需求设计",进入 code-require) |
| **E-16** | 模式 B 需求编码格式非法 | — | 沿用"两个目录都不存在"路径,整串视为需求内容 |
| **E-17** | 模式 B 模式识别歧义 | — | 路径感知无歧义(只检测目录存在性) |
| **E-18** | 无版本工作空间 | 屏显"未检测到激活的版本工作空间,先调 /code-version" + 退出码 3(沿用既有"步骤 0 失败"语义) | — |
...
- **`code-it`**:被驱动(FR-4 任务循环 + FR-5 派生任务循环;`code-it` 步骤 8.5 自含按需写单测,沿用)
...
- **REQ-00009**(V0.0.2):`code-unit` 守卫"项目可测性"(本技能依赖其判断"是否调 code-unit";`code-unit` 已于退场,守卫逻辑由 `code-it` 步骤 8a 接管)
