---
name: code-publish
description: 发布部署(长开发周期末使用)。一次调用告诉你"本版本能不能发":先做最严格的发布前检查(需求/任务/缺陷状态都得达标),通过后产出全新部署、版本升级、Q&A 三本手册骨架,供运维补全后执行。需要 `code-version` 之后、`code-check` 完成后调用;纯只读消费看板、不自动提交。
---
...
> **目的**:本需求顺带在项目级创建 `assistants/qanda/` 骨架。
