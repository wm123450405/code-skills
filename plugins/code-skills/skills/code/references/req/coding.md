...
- 测试失败 → 进入「错误修复循环(增强,整合运行时机制)」

#### 编译验证

1. 使用语言感知的构建命令(参考 `languages/<lang>.md`「构建命令检测」)
2. `Bash` 执行构建
3. 记录到 `compile-and-run.md`
4. 失败 → **先做运行环境/依赖边界判定**(见 `references/runtime-environment.md`「适用范围与术语」)
   - 失败信息指向"工具链本身不可用"(`command not found`、版本检测不通过等)→ 走「运行环境确认机制(新增)」
   - 失败信息指向"依赖包/库缺失"(`Cannot find package` / `No module named` / `package xxx does not exist` 等)→ 直接走对应包管理器安装,**不**走确认
   - 失败信息指向语法错误、链接错误等**代码 bug** → 进入「错误修复循环(增强,整合运行时机制)」

#### 运行验证

1. 使用语言感知的运行命令(参考 `languages/<lang>.md`「启动/运行命令检测」)
2. 启动运行,验证功能正常
3. 记录到 `compile-and-run.md`
4. 失败 → 同「编译验证」第 4 点,先做运行环境/依赖边界判定,再决定走「运行环境确认机制(新增)」还是错误修复循环

#### 测试验证(若适用)

1. 使用语言感知的测试命令(参考 `languages/<lang>.md`「测试框架识别」)
2. 执行测试,确保通过
...
> **仅当**「编译验证」「运行验证」「测试验证(若适用)」已经实际执行编译/运行/单测命令并确实因"运行时缺失"失败时,才触发本机制。区分边界详见 `references/runtime-environment.md`「适用范围与术语」。
>
...
确认提示模板与各选项处理详见 `runtime-environment.md`「确认提示模板(默认 / --confirm 模式)」。
...
按 `templates/req/TASK.md` 结构生成(模板见 `templates/req/TASK.md`,本文件不重复)。
...
loop:
  if 编译/启动/测试 有错误:
    // 步骤 0:判定错误类别 — 边界见 runtime-environment.md「适用范围与术语」
    if 运行时缺失:
      → 走「运行环境确认机制(新增)」:
...
- 运行时缺失:`command not found` 等工具链整体不可用 → 见「运行环境确认机制(新增)」与 `runtime-environment.md`
- 依赖包缺失:工具链在,但具体某个库/模块拉不到/装不上 → 直接走包管理器
- 代码 bug:逻辑错误、语法错误、空指针等 → 自行修复
- 设计缺陷:方案本身有问题,需调整设计 → 记录到 deviations.md,停下询问
- 网络/镜像源问题:归类按「适用范围与术语」区分(运行时下载失败 vs 依赖下载失败),按上面对应分支处理

## 阶段完成确认

> 三态确认模型:阶段边界确认由 `--confirm`/`--auto`/默认 控制。详见 common.md「阶段执行器」「交互确认」「--confirm 模式」。
