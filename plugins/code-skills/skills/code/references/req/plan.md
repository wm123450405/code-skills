...
> 本文件为 `/code req` 子命令的 PLAN 阶段提供详细流程。在进入 PLAN 阶段时加载。
...
> 三态确认模型:阶段边界确认由 `--confirm`/`--auto`/默认 控制。阶段内内容确认(任务拆分/优先级)仅在 `--auto` 模式下跳过。详见 common.md「阶段执行器」「交互确认」「--confirm 模式」。
...
你可以手动修改上述文件,完成后选择:
A. 继续(重新读取产出物,获取最新修改,进入 CODING 阶段)
B. 中止(保存当前进度,退出)
```

### --auto 模式

自动继续,屏幕输出 `[code req --auto] PLAN 完成,自动继续`。阶段内确认自动选推荐项,记录为"自动选择"。
