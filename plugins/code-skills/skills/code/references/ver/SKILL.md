---
name: code-ver
description: 版本管理与开发看板。仅首 token = `ver` 触发。例:`/code ver`(无参数,看开发进度)/ `/code ver V0.0.5`(切换或创建版本)/ `/code ver --publish`(发布检查,不切换)。看板模式只读,需写文件时切换到版本管理模式。
---
...
- **创建目录**时**必须**只在本技能工作空间(`assistants/<V>/`)或 `assistants/rules/` 内,不向其他位置创建版本子目录
