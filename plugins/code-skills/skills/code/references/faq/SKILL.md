---
name: code-faq
description: 跨版本知识查询与文档导出。仅首 token = `faq` 触发。例:`/code faq "用户登录在哪"`(查询)/ `/code faq --require REQ-00001 out.md`(导出需求)/ `/code faq --design REQ-00001 out.md`(导出设计)/ `/code faq --design REQ-00001 out.md --summary`(导出概要)/ `/code faq --template <tpl>`(模板填充)。
---

# `/code faq` — 知识查询

> 流程细节见 [`common.md`](common.md)。
...
- **导出模式下**(`/code faq --require/--design/--template`)**必须**只写用户指定输出路径,不得修改 `./assistants/` 下的任何文件
- **查询模式下**(`/code faq <关键词>`)**必须**只读,不调用 `Write`/`Edit`
- **无 `--require`/`--design` 时必须**仅屏显,不写出文件
- **`--summary` 必须**仅对 `--design` 生效;与 `--require` 同时指定时**必须**报错
- **导出 FAQ 内容时必须**只复制源文档原文,不补充任何额外内容;不得臆造
- **对 `./assistants/rules/` 必须**只读,不修改
- **`--template` 指定模板不存在时必须**原样输出(占位符未替换),不中断流程
- **修改仓库文件时必须**只改用户指定输出路径,不得越界
