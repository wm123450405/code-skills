...
>
> 在 CODING 阶段编译/单测步骤(`/code req` 流程的 `references/req/coding.md`「编译验证」「运行验证」「测试验证(若适用)」、「`/code fix` 流程的对应步骤)遇到"常规命令找不到运行环境"的场景时加载本文件。
>
...
| 失败信号 | 归类 | 处理方式 |
| --- | --- | --- |
| `command not found: go` / `'mvn' is not recognized` / `'python3' 不是内部或外部命令` | **运行环境缺失** | 走本文件「确认提示模板(默认 / --confirm 模式)」机制 |
...
| --- | --- | --- |
| 编译/测试运行失败,且归类为"运行环境缺失" | 弹出「确认提示模板(默认 / --confirm 模式)」,**等待用户决策** | 自动尝试通过包管理器或下载引导安装缺失的运行时,**无需询问** |
...
- **选 A**:用户通常会在自然语言回答里给出路径。如用户给出的是相对路径或简化名(如"go 装在 `~/sdk/go1.21/bin`"),先尝试 `where <runtime>` / `which <runtime>` 探测,若仍未找到再以用户回答为准,把后续命令改为绝对路径执行。**不**把用户提供的路径写入任何文档(详见「隐私约束(强制)」)。
...
- **选 C**:在 TASK-N.md 第 4 节表格里把"编译 / 运行 / 测试"改为 `用户跳过`,并在该行附简短原因(不含路径与具体环境值,见「隐私约束(强制)」)。后续编码循环仍然可以往下推进,代码修改也已经落地。
...
- "运行时检查结果"——枚举值:`configured` / `user-provided` / `auto-installed` / `skipped` / `unavailable`(5 项机器值,详见契约层 `references/_shared/contracts.md`「运行时状态(`TASK-N.md` 运行时行用)」);展示时再映射为中文:`已配置` / `由用户提供路径` / `由用户授权自动安装` / `由用户跳过` / `未配置`;**不允许**附加具体路径
- 上述枚举值在 TASK-N.md 第 4 节"验证结果"中以"运行时"行呈现(机器值直接写在表格中)
...
- 上游:`references/req/coding.md`「编译验证」「运行验证」「测试验证(若适用)」(`/code req` 与 `/code fix` 流程共用)
- 下游:`references/req/coding.md`「错误修复循环(增强,整合运行时机制)」/ 步骤 5 撰写 TASK-N.md
