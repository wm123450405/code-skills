...
- "运行时检查结果"——枚举值:`configured` / `user-provided` / `auto-installed` / `skipped` / `unavailable`(5 项机器值,详见契约层 `references/_shared/contracts.md` §2.4);展示时再映射为中文:`已配置` / `由用户提供路径` / `由用户授权自动安装` / `由用户跳过` / `未配置`;**不允许**附加具体路径
