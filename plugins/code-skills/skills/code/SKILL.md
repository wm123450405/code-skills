...
1. **子命令识别第一**:每次进入本技能,先看首 token(详见下方"子命令路由表"小节)
...
- 不要在主 SKILL.md 内塞子命令的详细流程(那是子命令 SKILL.md 的职责)
- 不要跳过 §0 不变式自检
- 不要调 `EnterPlanMode`
- 不要在 `req` / `fix` 的非 CODING 阶段修改 CWD 源码
- 不要重写既有 `./assistants/rules/<分类>.md`(走 `/code rule` 追加)
- 不要让 7 个子命令的 description 在 frontmatter 中语义重叠,避免触发歧义
